...
기본적으로 송출자 스피커에는 **원본**이 나오고 봇에만 필터본이 갑니다. 모니터를 켜면 원본+필터본이 **2중**으로 들립니다. 송출자도 *필터본만* 들으려면 원본을 귀에서 치워야 하는데 — **앱을 음소거하면 안 됩니다**(음소거하면 캡처까지 무음이 되어 송출·모니터 둘 다 죽습니다). 대신 **원본과 필터본을 서로 다른 출력으로 분리**하면 됩니다.

**방법 A — 출력장치가 2개 이상이면 (VB-CABLE 불필요)**

예: 스피커 + 헤드폰(또는 HDMI 모니터 스피커 등).
1. 그 앱(예: Chrome) **출력** → 내가 **안 쓰는 장치**(예: 스피커)
2. pepsistreamy **로컬 모니터 = 켜짐**, 모니터 **출력장치 = 내가 듣는 장치**(예: 헤드폰)
3. → 헤드폰엔 **필터본만**, 스피커엔 원본(안 들음). 가상 케이블 없이 끝.

**방법 B — 출력장치가 하나뿐이면 (VB-CABLE 사용)**

1. **Windows 설정 → 시스템 → 소리 → 볼륨 믹서** → 그 앱 → **출력**을 `CABLE Input` 으로 (음소거 ❌)
2. pepsistreamy **로컬 모니터 = 켜짐**, 모니터 **출력장치 = 내 스피커/헤드폰**

두 방법 모두 캡처 소스는 프로세스(그 앱) 또는 레거시(CABLE) 다 됩니다. 결과: 원본은 다른 출력으로 빠져 **안 들리고**(음소거 아님 = 캡처 살아있음), 모니터가 **필터본만** 내 장치로 재생 → **나도 청취자도 필터본만**. 캡처는 앱/CABLE, 모니터는 내 장치라 피드백도 없습니다.

| 내 환경 | 필터본만 듣기 |
|--------|------|
| 출력장치 2개 이상 | ✅ 방법 A (VB-CABLE 불필요) |
| 출력장치 1개 + VB-CABLE | ✅ 방법 B |
| 출력장치 1개 + VB-CABLE 없음 | ❌ (2중으로만 들림 — 모니터 끄고 원본만 듣거나 VB-CABLE 설치) |

> ⚠️ 음소거는 안 되는 이유: WASAPI 프로세스 루프백은 앱을 볼륨 믹서에서 뮤트하면 **캡처 스트림도 무음**이 됩니다(post-volume, 실측 확인). 그래서 "뮤트로 원본만 끄기"는 동작하지 않습니다. 반드시 **출력장치 분리**로 하세요.
