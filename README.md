...
기본적으로 송출자 스피커에는 **원본**이 나오고 봇에만 필터본이 갑니다. 모니터를 켜면 원본+필터본이 **2중**으로 들립니다. 송출자도 *필터본만* 들으려면 원본을 귀에서 치워야 하는데 — **앱을 음소거하면 안 됩니다**(음소거하면 캡처까지 무음이 되어 송출·모니터 둘 다 죽습니다). 대신 **앱의 출력 장치를 바꾸세요**:

1. **Windows 설정 → 시스템 → 소리 → 볼륨 믹서** → 그 앱(예: Chrome) → **출력**을 `CABLE Input` 으로 변경 (음소거 ❌, VB-CABLE 필요)
2. pepsistreamy: **로컬 모니터 = 켜짐**, 모니터 **출력장치 = 내 스피커/헤드폰**
3. 캡처 소스 = 프로세스(그 앱) 또는 레거시(CABLE) — 둘 다 가능

결과: 앱 원본은 CABLE 로 빠져 **안 들리고**(음소거가 아니라 캡처는 살아있음), 모니터가 **필터본만** 내 스피커로 재생 → **나도 청취자도 필터본만**. 캡처는 앱/CABLE, 모니터는 스피커라 피드백도 없습니다.

> ⚠️ 음소거는 안 되는 이유: WASAPI 프로세스 루프백은 앱을 볼륨 믹서에서 뮤트하면 **캡처 스트림도 무음**이 됩니다(post-volume). 그래서 "뮤트로 원본만 끄기"는 동작하지 않습니다. 반드시 **장치 라우팅(VB-CABLE)** 으로 하세요.
